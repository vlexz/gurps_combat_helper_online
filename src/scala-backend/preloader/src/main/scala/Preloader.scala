...
object Preloader extends App {
  private val parseInt = (t: String) => t match {
    case "" => 0
    case x => x.toInt
  }
  private val parseDouble = (t: String) => t match {
    case "" => 0.0
    case x => x.toDouble
  }
  for (adv <- (XML load (getClass getResourceAsStream "/adv.xml")) \ "advantage") yield Trait(
    name = (adv \ "name").text,
